...
class Doc {
	...
	constructor(id)																								// CONSTRUCTOR
	{
		...
		if (id == "preview") {																						// Preview instructions
			...
			this.LoadCSV(this.courseId);																			// Load default lobs
...
/// MISC //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	LoadCSV(url) 																								// LOAD DOC FROM CSV FILE
	{
		$.ajax(url).done(handleCSVResponse) 																		// Load file

		function handleCSVResponse(res) {																			// HANDLE INCOMING DATA
			let i,v,s,d=[];
			res=res.replace(/\r/g,"");																				// Remove CRs
			let lines=res.split("\n");																				// Split into lines
			for (i=1;i<lines.length;++i) {																			// For each line
				v=lines[i].split(",");																				// Split by comma																																				
				s=(v[0]+v[1]+v[2]+v[3]).length+5;																	// Length of 1st 4 fields																				
				d.push([ v[0],v[1],v[2],v[3],lines[i].substr(s).slice(0,-1)] );										// Add array of 5 fields
				}
			app.doc.InitFromJSON(d);																				// Init project
			}
		}
